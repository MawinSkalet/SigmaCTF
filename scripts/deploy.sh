...
[[ ! -f deploy/traefik/dynamic/active.yaml ]] || cp deploy/traefik/dynamic/active.yaml deploy/state/previous-route.yaml
python3 - <<'PY'
import json, os
...
os.replace(directory/'active.tmp',directory/'active.yaml')
PY
# Poll through the proxy after its file-provider debounce; preserve old slot on failure.
healthy=false
for attempt in $(seq 1 15); do
  res=$(curl --silent -H "Host: $APP_HOST" http://127.0.0.1:8080/api/health 2>/dev/null || true)
  if python3 -c 'import json,sys,os; sys.exit(0 if json.loads(sys.argv[1]).get("release")==os.environ["IMAGE_TAG"] else 1)' "$res" 2>/dev/null; then
    healthy=true; break
  fi
  sleep 2
done
if [[ $healthy != true ]]; then
  if [[ -f deploy/state/previous-route.yaml ]]; then
    cp deploy/state/previous-route.yaml deploy/traefik/dynamic/restore.tmp
    mv deploy/traefik/dynamic/restore.tmp deploy/traefik/dynamic/active.yaml
  fi
