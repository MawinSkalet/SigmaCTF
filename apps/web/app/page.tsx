...
export default function Arena(){
  ...
  const refresh=async()=>{
    try{
      const [c,b,a,p]=await Promise.all([api('/challenges'),api('/scoreboard'),api('/announcements'),api('/auth/providers').catch(()=>({google:false}))]);
      setCatalog(c.challenges);setBoard(b);setAnnouncements(a.announcements||[]);setProviders(p);setError('');
    }
  ...
  return <div className="shell">
    ...
    <dialog ref={authDialog} onCancel={()=>setAuthOpen(false)} aria-labelledby="auth-title">
      <div className="modal-content">
        ...
        {providers.google&&<div style={{marginBottom:'20px'}}>
          <a href="/api/auth/google" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'10px',background:'#fff',color:'#111827',padding:'12px',width:'100%',font:'600 13px monospace',border:'1px solid #e5e7eb',textDecoration:'none',boxShadow:'0 2px 5px rgba(0,0,0,0.2)'}}>
          ...
          <div style={{display:'flex',alignItems:'center',margin:'18px 0 10px',gap:'10px'}}>
            <div style={{flex:1,height:'1px',background:'var(--line)'}}/>
            <span style={{font:'10px monospace',color:'var(--muted)',letterSpacing:'1px'}}>OR USE HANDLE</span>
            <div style={{flex:1,height:'1px',background:'var(--line)'}}/>
          </div>
        </div>}
